...
1. Gather. Run `readmerlin context --format md` and read what it prints. It lists the skills, commands, agents, manifests, hosts, workflows and the existing README in full. Do not walk the source tree.
...
6. Write the parts in order: hero, Features, In action, Fit, How it compares, Security and limits, Badges. A part with nothing to say is dropped. Derive the tagline from the outcome, not the file tree. When a README exists, it is the draft: keep its tagline, its feature blocks, its comparison rows and its checklist items word for word, and change a sentence only when a fact moved, a rule fails, or the user asks. Say which sentence changed and why. Show each section in chat before writing it to disk.
