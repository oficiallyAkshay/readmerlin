...
3. Draw first. Name the product's verb before anything else: gather, improve, watch, translate. The hero takes that shape, in the reader's nouns, and another project's layout is never reused for a different verb. Write `<name>.hero.json` beside the SVG and draw it with `scripts/hero-svg.mjs` in this skill's folder. It has one layout per verb, and a new verb gets a new layout in the script. The herofold skill draws the same spec when the user has it. Never use Mermaid. Look at the hero at full width and in dark theme before moving on.
