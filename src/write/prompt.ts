...
export function buildPrompt(args: { rules: string; contextMd: string; existing?: string; instructions?: string }): string {
  const parts = [
    ...
    "",
    "Sections, in this order and no others: the hero, Features, Badges (only when the project ships badges), Security, How it compares, Callouts. No Quick start section and no section for agents: the install step is one plain sentence in the hero, and the agent block belongs in CONTRIBUTING. No code fence, no shell command and no link to a YAML file anywhere.",
    "Visuals: reference the hero graphic at assets/readme/hero.svg, directly under the tagline and above the badge rows. Do not draw it. It is rendered later by the herofold skill from a committed spec.",
    "Badges: use img.shields.io with a logo parameter on every badge, and wrap every badge in a link. No CI status badge, and never a coverage badge for a service the repo does not use. When the context lists published packages, carry each one's version and downloads badges exactly as given, linked to the registry page.",
    "How it compares: head every column with a linked owner/repo name, this project first, and only state what you know from that project's README. Cells are \u2705, \u274c, or one or two words.",
