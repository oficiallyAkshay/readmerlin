...
`init-workflow` writes `.github/workflows/readme-check.yml`, which runs the rules on every push, pull request and once a week through the action. The action runs the script the skill carries from its own checkout, so the commit a workflow pins is exactly the code that runs. Pin a commit of this repository's latest release, with a `# vX.Y.Z` comment naming that release so the pin is verifiable; `init-workflow` resolves and fills in the current one. It only checks; the writing is the agent's job.

For a repo with no registry package, clones are the only count there is. `init-workflow --clones` also writes `.github/workflows/clonometer.yml`, the consumer workflow of [clonometer](https://github.com/oficiallyAkshay/clonometer) pinned the same way, to the commit of its own latest release. It needs a `TRAFFIC_TOKEN` secret: a fine-grained token scoped to the one repository, with Contents write and Administration read. A private repository needs clonometer's gist storage instead, set by hand from its README, since the branch's numbers file is not public.

This repo's own `.github/workflows/ci.yml` runs a `checks` job that runs the same hooks as `.pre-commit-config.yaml` (gitleaks, actionlint, zizmor), verifies every `uses:` is pinned to the SHA its version comment names with `pinact-action`, and scans the full history for secrets; a `test` job that runs the node matrix, typecheck, build, the test suite and the self check; and a `ci` gate job that needs both and only passes when every job it needs succeeded. Branch protection requires only the `ci` job, so that gate is the one check to keep green.

Three more workflows run outside that gate, so a finding in any of them is triaged, not a merge blocker. `.github/workflows/dependency-review.yml` scans a pull request's manifest changes for known-vulnerable packages, dev dependencies included, and comments on the pull request if it finds one. `.github/workflows/audit.yml` runs `npm audit --audit-level=low` weekly and on any pull request that touches `package-lock.json`. `.github/workflows/dependabot-auto-merge.yml` arms auto-merge on every Dependabot pull request, since coverage is enforced at 100 percent and every action is SHA-pinned; the branch ruleset's required `ci` check still has to pass before GitHub merges anything.

Dependabot, configured in `.github/dependabot.yml`, opens weekly pull requests for GitHub Actions and for npm dependencies so the workflow pins and dev tooling do not go stale.
